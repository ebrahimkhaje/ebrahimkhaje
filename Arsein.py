...
class Robot_Rubika:
	...
	def _getPwa():
	    return choice(Server.pwaadrass)
	...
	def sendcodepwa(self):
	    inData = {"method":"sendCode","input":{"phone_number":"989223709512","send_type":"Internal"},"client":SetClines.PWA}
	    while 1:
	        try:
	            return loads(self.enc.decrypt(loads(request.urlopen(request.Request(Robot_Rubika._getPwa(), data=dumps({"api_version":"5","auth": self.Auth,"data_enc":self.enc.encrypt(dumps(inData))}).encode(), headers={'Content-Type': 'application/json'})).read()).get('data_enc')))
	        except:continue
